...
- **Cross-Origin Support (CORS):** Backend supports Cross-Origin Resource Sharing, allowing the API to be accessed from authorized external domains. Configurable via environment variables for flexible deployment scenarios.
...
- **Article Management:** Retrieve articles with flexible filtering. Items are consistently sorted by creation date in ascending order (oldest first) across all views to ensure a predictable reading sequence. Track article status including read/unread. Supports efficient delta synchronization for read statuses via a dedicated API, allowing clients to fetch only the changes since their last update. Includes periodic background synchronization to ensure read states remain consistent across sessions and devices. Features an information-dense list view displaying key metadata (publication/creation dates, source domain, snippets). The display order for metadata is standardized across the application, starting with the source domain, followed by the publication date and the discovery ("Received") date. Includes date-based filtering (e.g., Past 24 hours, Past 7 days, Past 30 days, Past 90 days, or Past 365 days) and a visibility toggle to easily show or hide read items. Supports bulk operations, such as marking multiple articles as read simultaneously for improved efficiency.
  ...
  - **Transient Removal:** Features a "Clear Read Items" button to temporarily remove read articles from the current view. This memory-only operation maintains the "cleared" state throughout the browser session, ensuring items stay hidden even after manual or automatic refreshes, without permanently modifying the backend database. The state is reset upon browser reload.
  - **Adaptive Metadata Display:** Both the list and detail views automatically adjust their metadata display. On narrow viewports (< 480px), "Published" and "Received" text labels are replaced by intuitive icons with tooltips to maintain a compact, information-rich layout. The metadata row supports wrapping for long content (like domain names), with publication and discovery dates grouped to ensure they wrap together as a single unit, preventing horizontal overflow while maintaining readability.

- **Content Reading:** Clean, distraction-free modal interface for reading full article content. Supports rich HTML rendering, images, source domain, author attribution, and categories. Features a highly optimized, compact header to maximize the vertical display area for the article. The primary action, marking an item as read or unread, is accessible via an ergonomic Floating Action Button (FAB) at the bottom-right, keeping the main content clear of obstruction. Features robust, keyboard-centric navigation between items with automatic pagination and read-status management, including dedicated keyboard shortcuts for toggling read status (`m` / `M`) and skipping to the next item while preserving the unread state (`n` / `N`), intentionally omitting redundant on-screen navigation buttons to further focus on content. State changes are announced to screen readers via ARIA live regions for enhanced accessibility. The navigation sequence is synchronized with the current list view, including items that have been marked as read during the session. Includes a virtual placeholder at the end of lists to provide a clear terminal point and streamline marking the final item as read. Implements standard modal interactions including Escape key and backdrop dismissal. Precise interaction targets for embedded media ensure that click and touch actions are restricted to the visible boundaries of images, preventing accidental triggers when interacting with surrounding whitespace. Full URL synchronization (deep linking) for direct access and browser history support, ensuring that active filters (tags and date ranges) are preserved when returning to the list view.
  ...
  - **Swipe Navigation:** Intuitive horizontal swipe gestures on touch devices allow users to navigate between articles seamlessly, with smooth visual feedback as the content follows the drag. An upward swipe gesture allows skipping to the next item while preserving the current item's unread state. Features a non-linear resistance bounce effect when swiping past the first or last items to provide tactile feedback, and includes ARIA-compliant instructions for screen reader users.
