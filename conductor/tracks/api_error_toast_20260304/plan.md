...
- [ ] Task: Update `frontend/src/lib/toast.tsx` to expose a global `toast` object with `show`, `dismiss`, and `toasts`. This allows calling `show` from outside of SolidJS components (like API interceptors).
- [ ] Task: Ensure `ToastProvider` is updated to utilize this global reactive state so that existing `useToast` hooks still function correctly.
