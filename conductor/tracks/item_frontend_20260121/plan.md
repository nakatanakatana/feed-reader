# Implementation Plan: Item Fetching and Display Frontend

Implement the frontend components and logic for listing items, filtering by feed, and managing item status according to the `spec.md`.

## Phase 1: Foundation & Data Access [checkpoint: 32f2efc]
Setup the necessary queries and basic state management for items.

- [x] Task: Define Item query keys and basic fetching logic using TanStack Query and ItemService.ListItems. 1eb0934
- [x] Task: Implement useItems hook to handle fetching, pagination (load more), and filtering by feed_id. aae7160
- [x] Task: Implement useUpdateItemStatus mutation hook using ItemService.UpdateItemStatus. 1066deb
- [x] Task: Conductor - User Manual Verification 'Phase 1: Foundation & Data Access' (Protocol in workflow.md) 32f2efc

## Phase 2: Item List Components [checkpoint: 0f1679d]
Create the UI components for displaying items in a list view.

- [x] Task: Create ItemRow component for the list view, displaying title, metadata, and read status toggle. dbaf85a
- [x] Task: Implement "Mark as Read/Unread" toggle logic in `ItemRow`. c3f261e
- [x] Task: Implement "Open URL" functionality in `ItemRow`. c3f261e
- [x] Task: Create `ItemList` component that renders a list of `ItemRow`s and the "Load More" button. 92d25db
- [x] Task: Conductor - User Manual Verification 'Phase 2: Item List Components' (Protocol in workflow.md) 0f1679d

## Phase 3: Routing & Views [checkpoint: c1cde31]
Integrate the item list into the application's routing.

- [x] Task: Update the "All Items" view (e.g., `feeds.tsx` or a new index route) to use `ItemList`. 0f1679d
- [x] Task: Update the "Feed Specific" view to pass the `feed_id` from the URL to `ItemList`. b4a821c
- [x] Task: Ensure navigation between different feeds updates the item list correctly. b4a821c
- [x] Task: Conductor - User Manual Verification 'Phase 3: Routing & Views' (Protocol in workflow.md) c1cde31

## Phase 4: Refinement & Mobile Optimization [checkpoint: a4699c6]
Polish the UI and ensure it works well on all devices.

- [x] Task: Apply styling using Panda CSS to ensure the list view is clean and responsive. c1cde31
- [x] Task: Add loading and error states for the item list and "Load More" action. 92d25db
- [x] Task: Verify touch targets and responsiveness on mobile-sized viewports. a4699c6
- [x] Task: Conductor - User Manual Verification 'Phase 4: Refinement & Mobile Optimization' (Protocol in workflow.md) a4699c6
