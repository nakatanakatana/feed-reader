...
- [x] Task: Create a custom hook or utility function to prefetch a list of items using TanStack Query's `prefetchQuery`.
- [~] Task: Implement logic to calculate the window of 3 items before and after the current item based on the active item list.
- [x] Task: Integrate the prefetch trigger into `ItemDetailRouteView` or `ItemDetailModal` so it fires on mount and when the item ID changes.
- [x] Task: Ensure that the prefetch logic handles the "end of list" and "start of list" cases gracefully.
