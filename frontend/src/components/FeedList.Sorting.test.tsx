...
import { ListTagsResponseSchema, TagService } from "../gen/tag/v1/tag_pb";
  ...
  it("sorts feeds correctly by title", async () => {
    worker.use(
      ...
      mockConnectWeb(TagService)({
        method: "listTags",
        handler: () => create(ListTagsResponseSchema, { tags: [] }),
      }),
      mockConnectWeb(FeedService)({
        method: "listFeedTags",
        handler: () => create(ListFeedTagsResponseSchema, { feedTags: [] }),
      }),
    );
