...
import { filterAndSortItems, SortOrder, type Item } from './item-utils';
...
describe('item-utils PBT', () => {
  ...
  const filtersArbitrary = fc.record({
    ...
    sortOrder: fc.constantFrom(SortOrder.UNSPECIFIED, SortOrder.ASC, SortOrder.DESC),
  });

  it('should always satisfy filtering invariants', () => {
    fc.assert(
      fc.property(fc.array(itemArbitrary), filtersArbitrary, (items, filters) => {
        const result = filterAndSortItems(items, filters);

        // 1. Result should only contain items from the original list
        expect(result.every(item => items.some(i => i.id === item.id))).toBe(true);

        // 2. Result should not contain items that don't match the filters
        if (filters.feedId) {
          expect(result.every(item => item.feedId === filters.feedId)).toBe(true);
        }
        if (filters.isRead !== undefined) {
          expect(result.every(item => item.isRead === filters.isRead)).toBe(true);
        }
        if (filters.isSaved !== undefined) {
          expect(result.every(item => item.isSaved === filters.isSaved)).toBe(true);
        }
      })
    );
  });
