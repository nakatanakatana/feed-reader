...
function createUnaryFn(transport, service, method) {
  return function(requestMessage, callback, options) {
    ...
    transport.unary(service, method, abort.signal, options.timeoutMs, options.headers, requestMessage, options.contextValues).then((response) => {
      var _a, _b;
  ...
  if (error) {
    ...
    if (error.details.length > 0) {
      const status = new Status({
        ...
        details: error.details.map((value) => "getType" in value ? Any.pack(value) : new Any({
    ...
    case MethodKind.Unary:
      return function unary(input) {
        return __asyncGenerator3(this, arguments, function* unary_1() {
          ...
          const next = applyInterceptors(anyFn, interceptors);
          const { message, header, trailer } = yield __await3(next({
            init: {
              method: context.requestMethod
            },
...
function createHandler2(opt, spec) {
  ...
  return async function handle(req) {
    ...
    const timeout = parseTimeout(req.header.get(headerTimeout), opt.maxTimeoutMs);
    const context = createHandlerContext(Object.assign(Object.assign({}, spec), { requestMethod: req.method, protocolName: protocolName2, timeoutMs: timeout.timeoutMs, shutdownSignal: opt.shutdownSignal, requestSignal: req.signal, requestHeader: req.header, url: req.url, responseHeader: {
...
function createUnaryHandler(opt, spec, serialization) {
  return async function handle(req) {
    ...
    const context = createHandlerContext(Object.assign(Object.assign({}, spec), { requestMethod: req.method, protocolName: protocolName3, timeoutMs: timeout.timeoutMs, shutdownSignal: opt.shutdownSignal, requestSignal: req.signal, requestHeader: req.header, url: req.url, responseHeader: {
...
function requestHeader(methodKind, useBinaryFormat, timeoutMs, userProvidedHeaders, setUserAgent) {
  ...
  if (setUserAgent && !result.has(headerUserAgent2)) {
    result.set(headerUserAgent2, "connect-es/1.7.0");
  }
  return result;
}
...
//# sourceMappingURL=chunk-JZFTH5AK.js.map
